...
public class FileProcessing
{
    ...
    private async Task GenerateSingleOutputAsync(
        HttpClient http,
        string model,
        string vsId,
        string jobId,
        ...
        )
    {
        ...
        try
        {
                ...
                if (Directory.Exists(chunksDir))
                    {
                        ...
                        Exporting.RunProcess("pandoc", $"\"{overviewPath}\" -o \"Replybrary_Overview.docx\" --toc", outDir);
                ...
                if (Directory.Exists(chunksDir))
                    {
                        ...
                        Exporting.RunProcess("pandoc", $"\"{path}\" -o \"Replybrary_Screen_workflow_mapping.docx\" --toc", outDir);
                        Console.WriteLine($"Wrote: {wordPath}");
            ...
            {
                throw new Exception($"Unknown generate type. Use: overview | workflows | faq | diagrams | erd | screen-mapping | environment-variables");
            }
        }
        catch (Exception ex)
        {
            Console.Error.WriteLine(ex);
            _jobs.UpdateSingleOutputFileProgress(jobId, cmd, JobState.Failed);
        }
    }
    ...
    public async Task<string> ProcessFile(List<string> SelectedOutputTypes, string jobId)
    {
        ...
        try
        {
            ...
            var envMapFlag = Path.Combine(Directory.GetCurrentDirectory(), "..", "backend.Application", "LLM", "envmap.replybrary.json");
            ...
            for (int type = 0; type < SelectedOutputTypes.Count; type++)
            {
                ...
                generationTasks.Add(
                    GenerateSingleOutputAsync(http, model, vsId, jobId, rawCmd, chunksDir, outDir, envMap, envMapText));
            }
